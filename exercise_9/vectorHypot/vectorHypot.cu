 ...
 * Extended by Mitch Stark in CS 374, Fall 2017.
 * Purpose is to calculate the hypotenuse of A and B and store the result in C
 */
...
int main(int argc, char** argv)
{
    ...
    double copy_to_device_start, copy_to_device_end;
    double compute_start, compute_end;
    double copy_back_start, copy_back_end;
   ...
   // printf("[Vector addition of %lu elements]\n", numElements);
    ...
    copy_to_device_start = omp_get_wtime();
    ...
    copy_back_start = omp_get_wtime();
    err = cudaMemcpy(h_C, d_C, size, cudaMemcpyDeviceToHost);
    copy_back_end = omp_get_wtime();
    checkErr(err, "Failed to copy vector C from device to host");

    total_end = omp_get_wtime();
    printf("Copy to Device Time: %lf\n", copy_to_device_end - copy_to_device_start);
    printf("Compute time: %lf\n", compute_end - compute_start);
    printf("Copy back time: %lf\n", copy_back_end - copy_back_start);
