...
int main(int argc, char** argv) {
    ...
    unsigned long long numTrapezoids = processCommandLine(argc, argv), startPoint, chunkSize;
    ...
    startPoint = chunkSize * id + 1;
    if (id == numProcesses - 1) {
        chunkSize += (numTrapezoids % numProcesses) - 1;
    }
    
    approximatePI = integrateTrap(0.0, 1.0, numTrapezoids, chunkSize, startPoint, id) * 4.0;
